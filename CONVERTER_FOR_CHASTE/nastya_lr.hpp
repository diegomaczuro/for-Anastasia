    ...
    AbstractCardiacCell* CreateCardiacCellForTissueNode(Node<3>* pNode)
    {
        ...
        for(int idx=0; idx < MAGIC_NUMBER; idx++){
            if ((x - m(idx,1))*(x - m(idx,1)) + (y - m(idx,2))*(y - m(idx,2)) + (z - m(idx,3))*(z - m(idx,3)) < 0.15*0.15) { //TODO Тут менять ход волокон
                boost::shared_ptr <SimpleStimulus> mpStimulus(new SimpleStimulus(-100000.0, 2.0, 0.0));
                //TODO: Memory leak
...
public:
    ...
    void Test_nastya_tnnp06() throw(Exception)
    {
        ...
        VtkMeshWriter<3,3> vtk_writer("./nastya_lr/", "init_mesh_1", false);
        vtk_writer.WriteFilesUsingMesh(*p_mesh);

        VtkMeshWriter<3,3> vtk_writer2("./nastya_lr/", "init_mesh_2", false);
